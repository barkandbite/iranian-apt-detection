...
- **NEW (v4.0.24)**: **Seedworm/MuddyWater nine-country DLL sideloading campaign** (WatchGuard July 1 2026, Huntress corroboration) — signed Fortemedia `fmapp.exe`→`fmapp.dll` and SentinelOne `sentinelmemoryscanner.exe`→`sentinelagentcore.dll` sideloads embedding ChromElevator browser credential theft; Node.js implant chain; victims in 9 countries on 4 continents incl. week-long dwell at a South Korean electronics maker. 2 Suricata rules (SID 2000562-2000563): exfil staging via `sendit.sh` (DNS + TLS SNI, policy-grade, thresholded). 2 Wazuh host rules (101530-101531): both sideload pairings via Sysmon Event 7. C2 IP 157.20.182.49 already covered (SID 2000156/2000316/2000326/2000515).
...
- **FIXED (v4.0.18)**: SID 2000022/2000026 mixed legacy/sticky HTTP buffer syntax (Suricata 7.0+ load failure class)
- **NEW (v4.0.16)**: MuddyWater **Stagecomp/Darkcomp** staging IPs + ms_upd.exe dropper behavioral (SID 2000521-2000523)
- **NEW (v4.0.15)**: Prince of Persia (Infy) **Foudre replacement C2 IPs** (SID 2000519-2000520, SafeBreach)
- **NEW (v4.0.14)**: Iranian APT **cloud C2 domains** — somee.com, glitch.me, workers.dev free-tier abuse (SID 2000516-2000518, Trellix)
...
- **FIXED (v4.0.11)**: Archived deprecated CyberAv3ngers IOC file to resolve SID 2000496-2000497 collision
- **FIXED (v4.0.10)**: Merged CyberAv3ngers IOC rules into main file for daily sync compatibility
...
```bash
# Deploy consolidated rules (v4.0 — single file, 440 signatures)
...
### Rule Statistics
- **Wazuh Rules**: 281 detection rules across 12 files
- **Suricata Signatures**: 440 network signatures (1 consolidated rule file)
...
│   ├── iranian-apt-detection.rules  # Consolidated v4.0 (440 rules)
│   └── README.md
...
**Last Updated**: July 13, 2026 | **Version**: 4.0.24 | **Maintainer**: Bark&Bite Security Intelligence
