...
- **FIXED (v4.0.22)**: P0 — `iranian-apt-detection.rules` was rejected at load by Suricata 7.0.3 since v4.0.21 shipped (2026-06-11, 18 days of zero community coverage). SID 2000030 `dsize:>500000` exceeded uint16 max; SID 2000535 had `nocase` after `http.host`. Both fixed, behavior preserved. CI runner with `suricata -T` added so this can't ship silently again.
- **NEW (v4.0.22)**: 2 Wazuh host-side rules for MuddyWater Chaos false flag — `ms_upd.exe` Sysmon Event 1 (101528) + `Game.exe` Event 3 to moonzonet/uploadfiler (101529). Network IOCs already covered by v4.0.19.
...
```bash
# Deploy consolidated rules (v4.0 — single file, 429 signatures)
...
- **Suricata Signatures**: 429 network signatures (1 consolidated rule file)
...
│   ├── iranian-apt-detection.rules  # Consolidated v4.0 (429 rules)
│   └── README.md
